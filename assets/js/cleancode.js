...
function criarElementoSelect() {
  const allGroups = obterNomesDosGrupos();
...
// Função para obter todos os nomes dos grupos
async function obterNomesDosGrupos() {
  const response = await fetch(process.env.API_URL);
  const data = await response.json();
  return data.groups; // Assumindo que a resposta da API contém um array de grupos
}
